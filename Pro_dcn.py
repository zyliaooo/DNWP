...
import numpy as np
from Pro_utility import SEBlock


class DB(nn.Module):
    def __init__(self, planes, inplanes=None, stride=1):
        super().__init__()
        if inplanes is None:
            inplanes = planes
        self.conv1 = nn.Conv3d(inplanes, planes, 3, 1, 1, padding_mode='replicate')
        self.lrelu1 = nn.LeakyReLU(0.1, inplace=True)
        self.conv2 = deform_conv3x3x3(planes, planes, stride)
        self.lrelu2 = nn.LeakyReLU(0.1, inplace=True)
        self.conv3 = deform_conv3x3x3(planes, planes, stride)
...
class DB3Dconv(nn.Module):
    def __init__(self, planes, inplanes=None, stride=1):
        ...
        self.conv2 = deform_conv3x3x3(inplanes, planes, stride)
        self.bn2 = nn.BatchNorm3d(planes)
        self.lrelu2 = nn.LeakyReLU(inplace=True)
        self.conv3 = deform_conv3x3x3(planes, planes)
...
class DB1Dconv(nn.Module):
    def __init__(self, planes, inplanes=None, stride=1):
        super().__init__()
        if inplanes is None:
            inplanes = planes
        self.conv2 = nn.Conv3d(inplanes, planes, 3, 1, 1)
        self.bn2 = nn.BatchNorm3d(planes)
        self.lrelu2 = nn.LeakyReLU(inplace=True)
        self.conv3 = deform_conv3x3x3(planes, planes)
        self.bn3 = nn.BatchNorm3d(planes)
...
class DeformConv3d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1, bias=False):
        ...
        self.zero_padding = nn.ReplicationPad3d(padding)
        self.conv_kernel = nn.Conv3d(in_channels * N, out_channels, kernel_size=1, bias=bias)
        self.offset_conv_kernel = nn.Conv3d(in_channels, N * 3, kernel_size=kernel_size, padding=padding, padding_mode='replicate', bias=bias)

        self.mode = "deformable"
        self.init_offset()

    def init_offset(self):
        self.offset_conv_kernel.weight.data.zero_()
        # self.offset_conv_kernel.bias.data.zero_()
